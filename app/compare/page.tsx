...
// Dynamically import the heavy builder so its code never lands in the
// homepage bundle. The 'use client' directive inside CompareBuilder means
// it already crosses a server/client boundary, but without dynamic() the
// JS would still be eagerly included in the compare route chunk.
const CompareBuilder = dynamic(
  () => import('@/components/compare/CompareBuilder').then((m) => m.CompareBuilder),
  {
    ssr: false, // uses useRouter/usePathname — must run client-side only
    loading: () => (
      <div className="max-w-[1440px] mx-auto px-6 md:px-10 xl:px-20 py-32 lg:py-40">
        {/* Skeleton matches CompareBuilder layout to prevent layout shift */}
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-16 gap-6 animate-pulse">
          ...
          <div className="flex gap-2">
            {['Party', 'State', 'Constituency', 'Politician'].map((t) => (
              <div key={t} className="h-[36px] w-28 bg-white/[0.04] rounded-[18px] border border-white/[0.08]" />
            ))}
          </div>
        </div>
...
export default function ComparePage({
  ...
  return (
    <main className="min-h-screen bg-[var(--bg-base)]">
      <CompareBuilder initialSearchParams={searchParams} />
    </main>
  );
}
