...
export default function PromisesPage() {
  ...
  const avgConfidence = Math.round(PROMISES.reduce((s, p) => s + p.confidenceScore, 0) / PROMISES.length);
  ...
  const filteredPromises = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(p => p.title.toLowerCase().includes(q) || p.fullStatement.toLowerCase().includes(q));
    }
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-base)] pb-32">
      ...
      <div className="max-w-7xl mx-auto px-8 lg:px-12 pt-12 pb-10">
        ...
        <p className="text-[var(--text-secondary)] max-w-2xl leading-relaxed mb-12" style={{ fontSize: 'var(--font-body-lg)' }}>
          A forensic tracker of political commitments. Every promise is subjected to rigorous evidentiary standards.
        </p>

        {/* Stats Strip */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-0 border border-[var(--border-subtle)]">
          {[
            { label: 'Tracked',            value: PROMISES.length,                                                                     color: 'var(--text-primary)' },
            { label: 'Verified Complete',   value: PROMISES.filter(p => p.status === 'completed' || p.status === 'operational').length, color: 'var(--accent-positive)' },
            { label: 'Cancelled / Delayed', value: PROMISES.filter(p => p.status === 'cancelled' || p.status === 'delayed').length,    color: 'var(--accent-negative)' },
