      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-24">
        ...
        <div>
          <h2 className={clsx(
            ...
            politician.criminalCases.length > 0 ? "text-[var(--accent-negative)]" : "text-[var(--color-text-primary)]"
          )}>
          ...
          {politician.criminalCases.length === 0 ? (
            <div className="border border-[var(--border-subtle)] p-6">
              <div className="text-[var(--color-text-primary)] font-bold mb-1">No Criminal Cases Declared</div>
              <div className="text-[var(--color-text-tertiary)] text-sm">As per the most recent electoral affidavit (Form 26).</div>
            </div>
          ) : (
              ...
              {politician.criminalCases.map((c, i) => (
                <div key={i} className="border-b border-[var(--border-subtle)] pb-6">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-[var(--color-text-tertiary)] text-xs font-bold">{c.year}</span>
                  </div>
                  <div className="text-[var(--color-text-primary)] font-serif font-bold text-lg mb-2">{c.section}</div>
                  <div className="text-[var(--color-text-tertiary)] text-sm mb-3">{c.chargeDescription}</div>
                  <div className="flex items-center justify-between text-xs font-semibold text-[var(--color-text-tertiary)] uppercase tracking-wider">
                    <span>{c.court}</span>
                    <span className={clsx(
        ...
        {/* ===== ASSET DECLARATIONS ===== */}
        <div>
          <h2 className="text-xs font-bold uppercase tracking-widest text-[var(--color-text-primary)] mb-6">
            ...
            {politician.assetDeclarations.map((decl, i) => (
              <div key={i} className="border border-[var(--border-subtle)] p-6">
                <div className="flex items-center justify-between mb-6 pb-4 border-b border-[var(--border-subtle)]">
                  <span className="font-bold text-[var(--color-text-primary)] text-sm uppercase tracking-widest">{decl.year} Affidavit</span>
                ...
                <table className="data-table w-full">
                  <tbody>
                    <tr>
                      <td className="text-[var(--color-text-tertiary)] text-xs font-semibold uppercase pb-2">Total Assets</td>
                      <td className="text-right font-bold text-[var(--color-text-primary)] pb-2">{formatCurrency(decl.totalAssets)}</td>
                    </tr>
                    <tr>
                      <td className="text-[var(--color-text-tertiary)] text-xs font-semibold uppercase pb-4">Total Liabilities</td>
                      <td className="text-right font-bold text-[var(--accent-negative)] pb-4">{formatCurrency(decl.totalLiabilities)}</td>
                    </tr>
                    <tr>
                      <td className="text-[var(--color-text-tertiary)] text-xs font-semibold uppercase pt-4 border-t border-[var(--border-subtle)]">Net Worth</td>
